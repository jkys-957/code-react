const ResultModal = ({title, content, callbackFn}) => {
  return (
    <div className={`fixed top-0 left-0 z-[1055] flex h-full w-full justify-center bg-black bg-opcity-20`}
          onClick={() => {
            if (callbackFn) {
              callbackFn()
            }
      ...
      <div className="absolute bg-white shadow dark:bg-gray-700 opacity-100 w-1/4 rounded mt-10 mb-10 px-6 min-w-[600px]">
        ...
        <div className="flex justify-end">
          <button className="px-6 pt-4 pb-4 mt-4 text-lg text-white bg-blue-500 rounded"
                  onClick={() => {
                    if(callbackFn) {
                      callbackFn()
                    }
